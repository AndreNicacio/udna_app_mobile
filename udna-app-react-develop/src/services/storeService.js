...
const buyExamBilletService = async (user, exam) => {
  ...
  const examInfo = {
    id: exam.id,
    title: exam.title,
    price: maskRemoveService(discountService(exam.price, 0.8)),
  };
...
const getPrice = (card, exam) => {
  if (card.installments === 1) {
    return discountService(exam.price, 0.8);
  }
  return exam.price;
};
...
const mapStoreService = (storeData, storeType) => storeData.listCategorys.items.map((category) => ({
  id: category.id,
  type: category.type,
  name: category.name,
...
})).sort(sortItems).filter((sortedCategory) => sortedCategory.type === storeType);
