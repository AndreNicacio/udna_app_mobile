...
const ExamInfoContainer = () => {
  ...
  const goToConfirmation = async (url) => {
    try {
      ...
      if (supported) {
        ...
        showModalMessage({
          title: 'Ops!',
          description: 'Tivemos um problema ao realizar ao abrir o link de confirmação, tente novamente.',
        });
      }
    } catch (error) {
      showModalMessage({
  ...
  const buyBilletOrCard = async () => {
    ...
    if (error || res.data.buyExamCreditCard === null) {
      setLoading(false);
      setCard({
        ...card,
        valid: false,
      ...
      setLoading(false);
      goToConfirmation(res.data.buyExamCreditCard);
      navigate('Store');
    }
  };
